...
private:
	void edge_init_points(cv::Mat & img, int n, std::vector<cv::Point2d> & pts);
	void Hammersley(cv::Mat & img, int n, std::vector<cv::Point2d> & pts);
	void HammersleySequence(cv::Mat & img, int n, std::vector<cv::Point2d> & pts,size_t truncateBits);
